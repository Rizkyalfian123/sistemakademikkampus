...
export const AdminSidebar = ({ activeMenu, setActiveMenu, isOpen, setIsOpen, onLogout }) => {
      ...
      >
        <div className="h-20 flex items-center justify-between px-6 border-b border-gray-800">
           <div className="flex items-center gap-3">
              <div className="w-8 h-8 bg-blue-600 rounded-lg flex items-center justify-center shadow-lg font-bold text-white">A</div>
              <h1 className="font-bold text-lg tracking-wide text-white">Admin Panel</h1>
           </div>
        ...
        <div className="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
          {MENU_ITEMS.map((menu) => (
            ...
            >
              ...
              <span className={`ml-4 font-medium ${activeMenu === menu.id ? 'font-bold' : ''}`}>{menu.label}</span>
            </button>
          ))}
        </div>

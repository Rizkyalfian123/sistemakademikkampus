...
export default function Dashboard() {
  ...
  // A. Buka Form Input (Menerima ID, bisa berupa number '1' atau string '2.1')
  const handleOpenForm = (stageId) => {
    setCurrentStageId(stageId);
    setIsFormOpen(true);
  };

  // B. Simpan Data (CRUD) - DIPERBAIKI
  const handleSaveToDB = async (dataInput) => {
    try {
      console.log(`Menyimpan Data ke Local State (ID: ${currentStageId}):`, dataInput);
  ...
  return (
    <div className="flex h-screen bg-gray-50 font-sans text-gray-800 overflow-hidden relative">
      ...
      <Sidebar 
        ...
        isOpen={logic.mobileSidebarOpen}
        setIsOpen={logic.setMobileSidebarOpen}
                  ...
                  <div className="space-y-8">
                    ...
                    <div>
                      <div className="flex justify-between items-center mb-3">
                        ...
                        <span className={`px-4 py-1 rounded-full text-xs font-bold uppercase tracking-wide ${logic.taProgress?.percent === 100 ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700'}`}>
                          {logic.taProgress?.label}
                        </span>
                      </div>
                    ...
                    <div>
                      <div className="flex justify-between items-center mb-3">
                        ...
                        <span className={`px-4 py-1 rounded-full text-xs font-bold uppercase tracking-wide ${logic.magangProgress?.percent === 100 ? 'bg-green-100 text-green-700' : 'bg-purple-100 text-purple-700'}`}>
                          {logic.magangProgress?.label}
                        </span>
                      </div>
                      <div className="w-full bg-gray-100 rounded-full h-3 overflow-hidden">
              ...
              <div className="animate-pop-in">
                {logic.loadingAnnounce ? (
                  ...
                  <div className="bg-white rounded-3xl p-12 text-center border-2 border-dashed border-gray-300">
                    <p className="text-gray-400">Belum ada pengumuman saat ini.</p>
                  </div>
                ) : (
                    ...
                    {logic.announcements.map((item) => (
                      <div key={item.id} className="group bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-100 flex flex-col h-full overflow-hidden hover:-translate-y-1">
                        ...
                        <div className="p-6 flex-1 flex flex-col">
                          ...
                          <p 
                            className="text-gray-500 text-sm mb-6 flex-1 leading-relaxed" 
                ...
                <div className="space-y-6">
                  {logic.taStages.map(stage => (
                    <StageCard 
                      ...
                      // FIX: Passing reference, BUKAN eksekusi inline
      ...
      {/* 1. Chatbot */}
      <ChatWidget 
        announcements={logic.announcements} 
        taPercent={logic.taProgress?.percent} 
        magangPercent={logic.magangProgress?.percent}
      />
      
      {/* 2. Modal Pengumuman (Portal) */}
      {selectedAnnounce && (
        <AnnouncementModal 
          item={selectedAnnounce} 
          onClose={() => setSelectedAnnounce(null)} 
        />
      )}

      {/* 3. Modal Form Input (Portal) */}
      <FormModal 
          isOpen={isFormOpen}
